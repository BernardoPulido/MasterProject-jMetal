...
public class ANOVARunnerMO extends AbstractAlgorithmRunner {
    ...
    public void run() throws JMetalException, FileNotFoundException, IOException {
        ...
        for (ArrayList<Integer> combinacion : this.combinaciones)
        {
            ...
            if (!referenceParetoFront.equals("")) {
                printQualityIndicatorsForANOVA(population, referenceParetoFront); ;
            }

            //Calcular BestSolution de población
            double best = Double.POSITIVE_INFINITY;
            for(PermutationSolution<Integer> s : population){
                double fitness1 = (s.getObjective(0) - min1)/(max1-min1);
                double fitness2 = (s.getObjective(1) - min2)/(max2-min2);
                double fitness3 = (s.getObjective(2) - min3)/(max3-min3);
    ...
    public static void main(String[] args) throws JMetalException, FileNotFoundException, IOException {
        ANOVARunnerMO a = new ANOVARunnerMO();
        a.run();
    }
}
